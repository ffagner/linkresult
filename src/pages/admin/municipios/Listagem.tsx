...
import { listarMunicipios, excluirMunicipio } from '@/features/municipios/api'
...
export function MunicipiosListagem() {
  ...
  async function handleExcluir() {
    ...
    setSaving(true)
    try {
      await excluirMunicipio(deleting)
      toast('Município excluído com sucesso.')
      setDeleting(null)
      carregar()
    } catch {
      toast('Erro ao excluir município.', 'error')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div>
      <div className="mb-stack-lg flex items-center justify-between">
        <h1 className="text-headline-lg text-text-primary">Municípios</h1>
      ...
      {itens.length === 0 ? (
        ...
        <div className="overflow-hidden rounded-xl border border-border-technical bg-surface shadow-card">
          <div className="border-b border-border-technical px-gutter py-stack-md">
            <h3 className="text-headline-sm text-text-primary">Municípios</h3>
          </div>
            ...
            <table className="w-full text-left">
              <thead className="bg-surface-container-low">
                <tr>
                  ...
                  <th className="px-gutter py-4 text-label-sm text-text-secondary">ESTADO</th>
              ...
              <tbody className="divide-y divide-border-technical">
                {itens.map(item => (
                  <tr key={item.id} className="group transition-colors hover:bg-surface-container-low">
                    ...
                    <td className="px-gutter py-4">
                      <span className="text-body-sm text-text-secondary">{item.estado}</span>
                    </td>
                    <td className="px-gutter py-4">
      ...
      <ConfirmDialog
        ...
        message="Tem certeza que deseja excluir este município?"
        loading={saving}
